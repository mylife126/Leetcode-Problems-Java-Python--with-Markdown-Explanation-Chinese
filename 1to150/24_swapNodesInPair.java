...
1 2 3 4 5
dummy 1 2 3 4 5
举例， 对于 3 4 来说， 他们需要交换我们需要的nodes是 2 3 4， 因为2是prenode，它的next指针需要指向4，而4的next指向3，
3的next指向了4的next。

所以从一开始我们的current指在了dummy， swap（prenode = dummy) 
- > dummy 2 1 3 4 5, current 走去current.next.next也就是1， 因为我们是两个两个交换，所以每次走两步走到下一个prenode上 

swap(prenode = 1) -> dummy 2 1 4 3 5 同样现在把currentpointer指向curr.next.next = 3这个地方了，但是呢我们发觉current.next.next == null 所以停止， 
